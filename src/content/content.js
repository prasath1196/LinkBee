...
function getParticipantDetails(participant) {
    ...
    if (participant.messagingMember) {
        ...
        if (mini) {
            ...
            // distance is often in 'distance' field, checking schema...
            // fallback if not directly there:
        }
    }
    ...
    elements.forEach(element => {
        try {
            ...
            if (element.entityUrn && element.entityUrn.includes("fs_conversation")) {
                let urn = element.entityUrn;

                // Identify "Them" (Non-Me Participant)
                // Filter out "Me" by checking if not current user (if we knew ID), 
                // OR just take the first one that isn't me.
                // Simplified: Take first valid participant.
            ...
            // For delta sync, we often get single events of type 'fs_event'
                ...
                if (conversationUrn) {
                    ...
                    // For Delta updates, we might lack the full participant list to resolve the Legacy Key immediately.
                    // However, if the user opens the thread, we get the list.
                    // For now, we pass the Thread URN. 
                    // The Background Script or UI might need to map it if it only has the legacy ID.
                    // BUT: If the conversation exists in storage under Legacy ID, we should try to map it.
                    // We can't do lookups here easily.
                    // Solution: Send Thread URN, and let the UI/Background merge it if possible?
                    // OR: Rely on the fact that for extensive history, the 'list' endpoint is called.

                    conversations.push({
                        urn: conversationUrn.toString(), // We send Thread ID here for updates if we can't resolve Profile ID
